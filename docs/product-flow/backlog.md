...
| R6 | Validation | 订阅周期字段校验增强（`currentCycle >= 1`） | P1 | DONE | 非法周期输入被 ValidationPipe 拦截 |
| R7 | Migration | TSK-4.1 迁移安全补强（非空字段与历史数据回填策略） | P1 | DONE | 迁移前后数据完整，`prisma migrate` 与校验脚本通过 |
| T4.2 | Feature | Platform 模块（controller/service/dto + userId 隔离） | P2 | DONE | 完成平台 CRUD 与权限隔离测试 |
| T4.3 | Feature | ItemHistory CRUD（从 `POST /items/:id/histories` 起步） | P2 | DONE | 续费/维修/升级记录可独立维护 |
| T4.5 | Feature | Statistics 模块落地（对齐 Flutter 算法） | P2 | DONE | `/items/statistics` 与前端算法对齐验证通过 |
