...
- [x] 统计引擎：TCO 计算与日均成本统计已上线。
- [x] 自动化：基于 Cron 的订阅到期自动续费机制已启用。
- [x] 审计系统：全量业务审计与脱敏日志系统已上线。
- [x] 质量门：`lint/build` 基线已恢复可用。

### 已完成任务 (Resolved)
1. **统计引擎 (Statistics Engine)**：实现了日均成本和持有总成本算法。
2. **历史记录管理接口**：支持对维修、升级、续费记录的独立管理。
3. **过期检查机制**：使用 `@nestjs/schedule` 处理订阅状态流转。
4. **系统可观测性**：引入 Winston 结构化日志。
